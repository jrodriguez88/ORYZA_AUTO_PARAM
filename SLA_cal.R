...
LAI_df %>% select(-(`DVS       LAI`)) %>% na.omit() %>%
    ...
    geom_smooth(method = 'loess', se=F, linetype = "twodash") +
    theme_bw() +
    facet_grid(.~LOC_ID)
...
LAI_df %>% select(-(`DVS       LAI`)) %>% na.omit() %>%
    mutate(
        k = case_when(
            ...
            TRUE        ~ 0.5),
        CC = 1 - exp(-k*LAI)) %>% 
    ggplot(aes(DVS, CC)) + geom_point() + geom_smooth(se=F, linetype = "twodash") +
    facet_wrap(. ~ LOC_ID) +
    theme_bw() +
    labs(title = paste0("Canopy Cover (CC) - ", cultivar),
         x = "Development Stage (DVS)")

LAI <- LAI_df %>%select(-(`DVS       LAI`)) %>% na.omit() %>%
    left_join(BMASS_df) 

ggplot(LAI, aes(WLVG, LAI, label=ID)) +
...
ggplot(SLA, aes( DVS, SLA))+# +"F2000 SLA Loess", xlab="Development Stage", ylab="Specific Leaf Area")
    ...
    geom_line(data=data.frame(SLA=span75$fit, DVS=x), color="orange", linetype="dashed")+
...
ggplot(SLA, aes(DVS, SLA))+# +"F2000 SLA Loess", xlab="Development Stage", ylab="Specific Leaf Area")
           geom_point()+
    geom_smooth(se=F) +
    geom_line(data=data.frame(SLA=span50$fit+1.96*span50$se.fit, DVS=x), color="darkgreen", linetype="dashed")+
    geom_line(data=data.frame(SLA=span50$fit-1.96*span50$se.fit, DVS=x), color="red", linetype="dashed")+
    theme_bw()
...
SLA_df <- data.frame(DVS=round(x,2), SLA=round(span50$fit+1.96*span50$se.fit, 5))%>%
